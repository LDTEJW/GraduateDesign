...
# 渲染设置
plt.rcParams['figure.dpi'] = 300  # 默认 DPI 300
plt.rcParams['savefig.dpi'] = 300  # 保存时 DPI
plt.rcParams['figure.figsize'] = (16, 10)  # 默认图像尺寸
...
plt.rcParams['svg.fonttype'] = 'path'  # SVG 导出时字体转路径，避免字体缺失
...
class EnhancedFlightDataProcessor:
    ...
    def save_statistics(self, save_path='saved_models/statistics.pkl'):
        """保存训练数据中的统计映射（机场、航空公司、路线平均延误等）"""
        stats = {}

        # 机场统计
        for col in ['origin', 'destination']:
            if col in self.df.columns:
                stats[f'{col}_mean_delay'] = self.df.groupby(col)['arrival_delay'].mean().to_dict()
                stats[f'{col}_std_delay'] = self.df.groupby(col)['arrival_delay'].std().to_dict()
                stats[f'{col}_flight_count'] = self.df.groupby(col)['arrival_delay'].count().to_dict()

        # 航空公司统计
        if 'airline_code' in self.df.columns:
            stats['airline_mean_delay'] = self.df.groupby('airline_code')['arrival_delay'].mean().to_dict()
            stats['airline_std_delay'] = self.df.groupby('airline_code')['arrival_delay'].std().to_dict()
            stats['airline_flight_count'] = self.df.groupby('airline_code')['arrival_delay'].count().to_dict()
            stats['airline_dep_delay_mean'] = self.df.groupby('airline_code')['departure_delay'].mean().to_dict()

        # 路线统计
        if 'route' in self.df.columns:
            stats['route_mean_delay'] = self.df.groupby('route')['arrival_delay'].mean().to_dict()
            stats['route_std_delay'] = self.df.groupby('route')['arrival_delay'].std().to_dict()
            stats['route_flight_count'] = self.df.groupby('route')['arrival_delay'].count().to_dict()
            stats['route_mean_distance'] = self.df.groupby('route')['distance'].mean().to_dict()

        joblib.dump(stats, save_path)
        print(f"统计映射已保存到: {save_path}")
...
class HybridModelTrainer:
    ...
    def _prepare_sequences(self, seq_features, static_features, seq_length):
        """准备时序序列"""
        # 选择特征
        df_seq = self.processor.df[seq_features].copy()  # 只取特征，不包含 arrival_delay
        df_target = self.processor.df[['arrival_delay']].copy()  # 目标变量单独
        df_static = self.processor.df[static_features].copy() if static_features else None

        print(f"时序特征数据形状: {df_seq.shape}")
        if df_static is not None:
            print(f"静态数据形状: {df_static.shape}")

        # 只对时序特征标准化（不含目标变量）
        seq_scaler = RobustScaler()
        df_seq_scaled = seq_scaler.fit_transform(df_seq)  # shape: (n_samples, n_seq_features)

        # 对目标变量不做标准化，直接取原始值（但后续创建序列时需对齐）
        # 将目标变量转换为 numpy 数组
        target_values = df_target.values.reshape(-1, 1)
        ...
        self.scalers = {
            ...
            'seq_features': seq_features,
            'static_features': static_features if static_features else [],
            'seq_scaler_n_features_in': seq_scaler.n_features_in_,
            'static_scaler_n_features_in': static_scaler.n_features_in_ if static_scaler else 0,
            # 注意：不再保存目标变量相关的信息
        }
        ...
        for i in range(len(df_seq_scaled) - seq_length):
            # 时序特征序列（滑动窗口）
            X_seq.append(df_seq_scaled[i:i + seq_length, :])  # 形状 (seq_length, n_seq_features)
            # 静态特征（取窗口后一个时间点的静态特征）
            if df_static_scaled is not None:
                X_static.append(df_static_scaled[i + seq_length, :])
            else:
                X_static.append(np.zeros((1,)))
            # 目标变量（窗口后一个时间点的 arrival_delay 原始值）
            y.append(target_values[i + seq_length, 0])

        if len(X_seq) == 0:
            print("警告：没有足够的数据创建序列，减少序列长度")
            seq_length = min(10, len(df_seq_scaled) // 2)
            for i in range(len(df_seq_scaled) - seq_length):
                X_seq.append(df_seq_scaled[i:i + seq_length, :])
                if df_static_scaled is not None:
                    X_static.append(df_static_scaled[i + seq_length, :])
                else:
                    X_static.append(np.zeros((1,)))
    ...
    def train_xgboost(self, data):
        ...
        # 存储XGBoost训练历史
        if xgb_trainer.evals_result:
            self.results['xgb_history'] = xgb_trainer.evals_result
    ...
    def evaluate_models(self, data):
        ...
        results['集成模型'] = self._calculate_metrics(y_test, ensemble_pred.flatten())
    ...
    def analyze_by_delay_severity(self, data):
        ...
        for group_name, (lower, upper) in delay_severity_groups.items():
            ...
            if mask.sum() > 0:
                ...
                print(f"  集成模型 MAE: {ensemble_mae:.4f}")
    ...
    def visualize_results(self, data):
        ...
        fig.suptitle('模型性能对比分析', fontsize=24, fontweight='bold', y=0.995)
        ...
        # 获取模型列表，排除'T-Test'，因为它没有 MAE 等指标
        ...
        models_with_mae = [model_name_map.get(model, model) for model in models if 'MAE' in results[model]]

        if mae_values:  # 确保有数据
            colors = plt.cm.Set3(np.linspace(0, 1, len(mae_values)))
            bars1 = ax1.bar(range(len(mae_values)), mae_values, color=colors)
            ax1.set_xlabel('模型', fontsize=16, fontweight='medium')
            ax1.set_ylabel('MAE', fontsize=16, fontweight='medium')
            ax1.set_title('平均绝对误差 (MAE) 对比', fontsize=18, fontweight='bold')
            ax1.set_xticks(range(len(mae_values)))
            ax1.set_xticklabels(models_with_mae, rotation=0, fontsize=14)
        ...
        models_with_rmse = [model_name_map.get(model, model) for model in models if 'RMSE' in results[model]]
        ...
        if rmse_values:  # 确保有数据
            ...
            ax2.set_xlabel('模型', fontsize=16, fontweight='medium')
            ax2.set_ylabel('RMSE', fontsize=16, fontweight='medium')
            ax2.set_title('均方根误差 (RMSE) 对比', fontsize=18, fontweight='bold')
            ax2.set_xticks(range(len(rmse_values)))
            ax2.set_xticklabels(models_with_rmse, rotation=0, fontsize=14)
        ...
        models_with_r2 = [model_name_map.get(model, model) for model in models if 'R²' in results[model]]

        if r2_values:  # 确保有数据
            colors = plt.cm.Set3(np.linspace(0, 1, len(r2_values)))
            bars3 = ax3.bar(range(len(r2_values)), r2_values, color=colors)
            ax3.set_xlabel('模型', fontsize=16, fontweight='medium')
            ax3.set_ylabel(r'$R^2$', fontsize=18, fontweight='medium')
            ax3.set_title(r'决定系数 ($R^2$) 对比', fontsize=18, fontweight='bold')
            ax3.set_xticks(range(len(r2_values)))
            ax3.set_xticklabels(models_with_r2, rotation=0, fontsize=14)
            ...
            ax3.text(0.5, 0.5, r'无$R^2$数据', ha='center', va='center', fontsize=14)
            ax3.set_title(r'决定系数 ($R^2$) 对比', fontsize=16, fontweight='bold')

        # 4. MSE 对比
        ax5 = axes[1, 1]
        # 只选择有 MSE 指标的模型
        mse_values = [results[model]['MSE'] for model in models if 'MSE' in results[model]]
        models_with_mse = [model_name_map.get(model, model) for model in models if 'MSE' in results[model]]

        if mse_values:  # 确保有数据
            colors = plt.cm.Set3(np.linspace(0, 1, len(mse_values)))
            bars5 = ax5.bar(range(len(mse_values)), mse_values, color=colors)
            ax5.set_xlabel('模型', fontsize=16, fontweight='medium')
            ax5.set_ylabel('MSE', fontsize=16, fontweight='medium')
            ax5.set_title('均方误差 (MSE) 对比', fontsize=18, fontweight='bold')
            ax5.set_xticks(range(len(mse_values)))
            ax5.set_xticklabels(models_with_mse, rotation=0, fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig('picture/model_comparison.png', dpi=300, bbox_inches='tight', 
        ...
        # 检查是否有必要的模型数据
        required_models = ['LSTM', 'XGBoost', '集成模型']
        ...
        ax6.axis('off')
        plt.tight_layout()
        plt.savefig('picture/improvement_analysis.png', dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none', format='png')
        plt.show()
        ...
        try:
            print("计算集成模型误差...")
            errors['集成模型'] = y_test - ensemble_pred
        except Exception as e:
            print(f"计算集成模型误差失败：{e}")
            errors['集成模型'] = np.zeros_like(y_test)

        # 绘制误差分布图
        fig3, axes3 = plt.subplots(1, 3, figsize=(22, 6))
        fig3.suptitle('模型预测误差分布 (抽样显示)', fontsize=20, fontweight='bold', y=0.995)
        ...
        for idx, (model_name, error) in enumerate(errors.items()):
            ...
            # 误差分布
            ax.hist(error_clipped, bins=30, alpha=0.7, color='steelblue', edgecolor='black', linewidth=0.8)
            ...
            ax.set_xlabel('预测误差 (分钟)', fontsize=14, fontweight='medium')
            ax.set_ylabel('频数', fontsize=14, fontweight='medium')
            ax.set_title(f'{model_name} - 误差分布', fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig('picture/error_distributions.png', dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none', format='png')
        plt.show()
    ...
    def _visualize_t_test_results(self):
        ...
        plt.tight_layout()
        plt.savefig('picture/paired_t_test_results.png', dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none', format='png')
        plt.show()
    ...
    def _visualize_delay_severity_analysis(self):
        ...
        ensemble_mae = [analysis_results[group]['集成模型 MAE'] for group in groups]
        ...
        plt.tight_layout()
        plt.savefig('picture/delay_severity_analysis.png', dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none', format='png')
        plt.show()
        ...
        if '严重延误 (>60分钟)' in analysis_results:
            ...
            print(f"  集成模型MAE: {severe_results['集成模型 MAE']:.4f}")
    ...
    def _visualize_uncertainty_analysis(self):
        ...
        plt.tight_layout()
        plt.savefig('picture/uncertainty_analysis.png', dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none', format='png')
        plt.show()
        ...
        if '高不确定性' in analysis_results:
            ...
            print(f"  集成模型MAE: {high_results['集成模型 MAE']:.4f}")
            print(f"  简单平均法MAE: {high_results['简单平均法 MAE']:.4f}")
            print(f"  相对改进率: {high_results['相对改进率 (%)']:.2f}%")

    def plot_training_process(self):
        """可视化训练过程：LSTM损失/MAE曲线、XGBoost训练曲线、学习率曲线"""
        print("\n绘制训练过程图表...")

        fig, axes = plt.subplots(2, 2, figsize=(22, 14))
        fig.suptitle('模型训练过程可视化', fontsize=24, fontweight='bold', y=0.995)

        # ---- 子图1: LSTM 损失曲线 ----
        ax1 = axes[0, 0]
        if 'lstm_history' in self.results:
            lstm_hist = self.results['lstm_history']
            epochs = range(1, len(lstm_hist['train_loss']) + 1)

            ax1.plot(epochs, lstm_hist['train_loss'], 'o-', color='#1f77b4',
                     linewidth=2, markersize=5, label='训练损失 (Train Loss)', alpha=0.85)
            ax1.plot(epochs, lstm_hist['val_loss'], 's-', color='#ff7f0e',
                     linewidth=2, markersize=5, label='验证损失 (Val Loss)', alpha=0.85)

            # 标记最佳验证损失
            best_epoch = np.argmin(lstm_hist['val_loss']) + 1
            best_val = lstm_hist['val_loss'][best_epoch - 1]
            ax1.axvline(x=best_epoch, color='red', linestyle='--', linewidth=1.2, alpha=0.6)
            ax1.annotate(f'最佳: Epoch {best_epoch}\nVal Loss={best_val:.4f}',
                         xy=(best_epoch, best_val), xytext=(best_epoch + 1.5, best_val * 1.15),
                         fontsize=10, color='red', fontweight='bold',
                         arrowprops=dict(arrowstyle='->', color='red', alpha=0.6))

            ax1.set_xlabel('Epoch', fontsize=14, fontweight='medium')
            ax1.set_ylabel('Loss (Huber)', fontsize=14, fontweight='medium')
            ax1.set_title('LSTM 训练损失曲线', fontsize=16, fontweight='bold')
            ax1.legend(fontsize=11, loc='upper right')
            ax1.grid(True, alpha=0.3, linestyle='--', linewidth=0.8)
            ax1.set_xlim(1, len(epochs))
        else:
            ax1.text(0.5, 0.5, '无LSTM训练历史数据', ha='center', va='center',
        ...
        if 'lstm_history' in self.results:
            lstm_hist = self.results['lstm_history']
            epochs = range(1, len(lstm_hist['train_mae']) + 1)

            ax2.plot(epochs, lstm_hist['train_mae'], 'o-', color='#2ca02c',
                     linewidth=2, markersize=5, label='训练MAE (Train MAE)', alpha=0.85)
            ax2.plot(epochs, lstm_hist['val_mae'], 's-', color='#d62728',
                     linewidth=2, markersize=5, label='验证MAE (Val MAE)', alpha=0.85)

            # 标记最佳验证MAE
            best_mae_epoch = np.argmin(lstm_hist['val_mae']) + 1
            best_mae = lstm_hist['val_mae'][best_mae_epoch - 1]
            ax2.axvline(x=best_mae_epoch, color='red', linestyle='--', linewidth=1.2, alpha=0.6)
            ax2.annotate(f'最佳: Epoch {best_mae_epoch}\nVal MAE={best_mae:.4f}',
                         xy=(best_mae_epoch, best_mae), xytext=(best_mae_epoch + 1.5, best_mae * 1.15),
                         fontsize=10, color='red', fontweight='bold',
                         arrowprops=dict(arrowstyle='->', color='red', alpha=0.6))

            ax2.set_xlabel('Epoch', fontsize=14, fontweight='medium')
            ax2.set_ylabel('MAE', fontsize=14, fontweight='medium')
            ax2.set_title('LSTM 训练MAE曲线', fontsize=16, fontweight='bold')
            ax2.legend(fontsize=11, loc='upper right')
            ax2.grid(True, alpha=0.3, linestyle='--', linewidth=0.8)
            ax2.set_xlim(1, len(epochs))
        else:
            ax2.text(0.5, 0.5, '无LSTM训练历史数据', ha='center', va='center',
                     fontsize=14, transform=ax2.transAxes)
            ax2.set_title('LSTM 训练MAE曲线', fontsize=16, fontweight='bold')

        # ---- 子图3: XGBoost 训练曲线 ----
        ax3 = axes[1, 0]
        if 'xgb_history' in self.results:
            xgb_hist = self.results['xgb_history']

            # XGBoost evals_result 格式: {'train': {'rmse': [...], 'mae': [...]}, 'val': {...}}
            if 'train' in xgb_hist and 'val' in xgb_hist:
                # 优先使用 RMSE
                if 'rmse' in xgb_hist['train']:
                    train_metric = xgb_hist['train']['rmse']
                    val_metric = xgb_hist['val']['rmse']
                    metric_name = 'RMSE'
                elif 'mae' in xgb_hist['train']:
                    train_metric = xgb_hist['train']['mae']
                    val_metric = xgb_hist['val']['mae']
                    metric_name = 'MAE'
                else:
                    train_metric = []
                    val_metric = []
                ...
                if train_metric:
                    ...
                    ax3.plot(rounds, train_metric, '-', color='#1f77b4',
                             linewidth=1.5, label=f'训练{metric_name} (Train)', alpha=0.85)
                    ax3.plot(rounds, val_metric, '-', color='#ff7f0e',
                             linewidth=1.5, label=f'验证{metric_name} (Val)', alpha=0.85)

                    # 标记最佳验证指标
                    best_round = np.argmin(val_metric) + 1
                    best_val_metric = val_metric[best_round - 1]
                    ax3.axvline(x=best_round, color='red', linestyle='--', linewidth=1.2, alpha=0.6)
                    ax3.annotate(f'最佳: Round {best_round}\nVal {metric_name}={best_val_metric:.4f}',
                                 xy=(best_round, best_val_metric),
                                 xytext=(best_round + len(rounds) * 0.08, best_val_metric * 1.1),
                                 fontsize=10, color='red', fontweight='bold',
                                 arrowprops=dict(arrowstyle='->', color='red', alpha=0.6))

                    ax3.set_xlabel('Boosting Round', fontsize=14, fontweight='medium')
                    ax3.set_ylabel(metric_name, fontsize=14, fontweight='medium')
                    ax3.set_title(f'XGBoost 训练{metric_name}曲线', fontsize=16, fontweight='bold')
                    ax3.legend(fontsize=11, loc='upper right')
                    ax3.grid(True, alpha=0.3, linestyle='--', linewidth=0.8)
                    ax3.set_xlim(1, len(rounds))
                else:
                    ax3.text(0.5, 0.5, 'XGBoost训练指标数据为空', ha='center', va='center',
                             fontsize=14, transform=ax3.transAxes)
                    ax3.set_title('XGBoost 训练曲线', fontsize=16, fontweight='bold')
            else:
                ax3.text(0.5, 0.5, 'XGBoost训练历史格式异常', ha='center', va='center',
                         fontsize=14, transform=ax3.transAxes)
                ax3.set_title('XGBoost 训练曲线', fontsize=16, fontweight='bold')
        else:
            ax3.text(0.5, 0.5, '无XGBoost训练历史数据', ha='center', va='center',
                     fontsize=14, transform=ax3.transAxes)
            ax3.set_title('XGBoost 训练曲线', fontsize=16, fontweight='bold')

        # ---- 子图4: 学习率曲线 ----
        ax4 = axes[1, 1]
        if 'lstm_history' in self.results and 'lr' in self.results['lstm_history']:
            lstm_hist = self.results['lstm_history']
            lr_values = lstm_hist['lr']
            epochs = range(1, len(lr_values) + 1)

            ax4.plot(epochs, lr_values, 'D-', color='#9467bd',
                     linewidth=2, markersize=6, label='学习率 (Learning Rate)', alpha=0.85)

            # 填充学习率变化区域
            ax4.fill_between(epochs, 0, lr_values, alpha=0.15, color='#9467bd')

            # 标记学习率变化点
            prev_lr = lr_values[0]
            for i, lr in enumerate(lr_values):
                if lr != prev_lr:
                    ax4.annotate(f'{lr:.2e}',
                                 xy=(i + 1, lr), xytext=(i + 1, lr * 1.3),
                                 fontsize=9, color='#9467bd', fontweight='bold',
                                 ha='center',
                                 arrowprops=dict(arrowstyle='->', color='#9467bd', alpha=0.5))
                    prev_lr = lr

            ax4.set_xlabel('Epoch', fontsize=14, fontweight='medium')
            ax4.set_ylabel('Learning Rate', fontsize=14, fontweight='medium')
            ax4.set_title('LSTM 学习率变化曲线', fontsize=16, fontweight='bold')
            ...
            ax4.text(0.5, 0.5, '无学习率历史数据', ha='center', va='center',
                     fontsize=14, transform=ax4.transAxes)
            ax4.set_title('学习率变化曲线', fontsize=16, fontweight='bold')

        plt.tight_layout()
        plt.savefig('picture/training_process.png', dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none', format='png')
        plt.show()

        print("训练过程图表已保存到 picture/training_process.png")
...
def main():
    ...
    os.makedirs(os.path.join(script_dir, 'picture'), exist_ok=True)
    os.makedirs(os.path.join(script_dir, 'data_set'), exist_ok=True)  # 确保数据目录存在

    try:
        # 1. 数据加载和处理
        print("\n阶段1: 数据加载和处理")
        # 使用绝对路径加载数据集
        data_file_path = os.path.join(script_dir, 'data_set', 'flights_sample_3m.csv')
        ...
        # 保存数据特征信息
        feature_info = processor.save_feature_info(save_path=os.path.join(script_dir, 'saved_models', 'feature_info.pkl'))
        processor.save_statistics(os.path.join(script_dir, 'saved_models', 'statistics.pkl'))
        # 2. 创建训练器
        print("\n阶段2: 创建训练器")
        ...
        }, os.path.join(script_dir, 'saved_models', 'hybrid_lstm_model.pth'))

        # 保存XGBoost模型 (xgb.Booster)
        xgb_trainer.model.save_model(os.path.join(script_dir, 'saved_models', 'optimized_xgboost_model.json'))

        # 保存集成模型
        joblib.dump(ensemble_model, os.path.join(script_dir, 'saved_models', 'ensemble_model.pkl'))

        # 保存标准化器和特征信息
        joblib.dump(trainer.scalers, os.path.join(script_dir, 'saved_models', 'scalers.pkl'))

        # 保存完整的训练特征信息
        trainer.save_feature_info_file(os.path.join(script_dir, 'saved_models', 'feature_info.pkl'))

        # 保存标签编码器
        joblib.dump(processor.label_encoders, os.path.join(script_dir, 'saved_models', 'label_encoders.pkl'))
        print(f"标签编码器已保存到: {os.path.join(script_dir, 'saved_models', 'label_encoders.pkl')}")
        print(f"包含的编码器: {list(processor.label_encoders.keys())}")

        # 保存结果
        results_df = pd.DataFrame.from_dict(results, orient='index')
        results_df.to_csv(os.path.join(script_dir, 'data_set', 'model_comparison_results.csv'), encoding='utf8')

        # 保存t检验结果
        if 'T-Test' in results:
            t_test_df = pd.DataFrame([results['T-Test']])
            t_test_df.to_csv(os.path.join(script_dir, 'data_set', 'paired_t_test_results.csv'), encoding='utf8')
            print(f"配对样本t检验结果已保存到 {os.path.join(script_dir, 'data_set', 'paired_t_test_results.csv')}")

        # 保存延误程度分析结果
        if 'Delay Severity Analysis' in results:
            delay_df = pd.DataFrame.from_dict(results['Delay Severity Analysis'], orient='index')
            delay_df.to_csv(os.path.join(script_dir, 'data_set', 'delay_severity_analysis.csv'), encoding='utf8')
            print(f"延误程度分析结果已保存到 {os.path.join(script_dir, 'data_set', 'delay_severity_analysis.csv')}")

        # 保存不确定性分析结果
        if 'Uncertainty Analysis' in results:
            uncertainty_df = pd.DataFrame.from_dict(results['Uncertainty Analysis'], orient='index')
            uncertainty_df.to_csv(os.path.join(script_dir, 'data_set', 'uncertainty_analysis.csv'), encoding='utf8')
            print(f"不确定性分析结果已保存到 {os.path.join(script_dir, 'data_set', 'uncertainty_analysis.csv')}")
        ...
        # 检查集成模型是否优于单个模型
        if '集成模型' in results and 'LSTM' in results and 'XGBoost' in results:
            lstm_mae = results['LSTM']['MAE']
            xgb_mae = results['XGBoost']['MAE']
            ensemble_mae = results['集成模型']['MAE']
